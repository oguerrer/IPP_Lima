...
pero suponiendo que el presupuesto total crece a tasas menores que la
reflejada en los datos históricos (9.5%).

Se simula con tasas de crecimiento del 9, 8, ..., -5% anual y se la dinámica
...
for interest in range(-5, 10, 1):
    ...
    dfi.groupby('indicador').mean().to_csv(home+"data/sims/decrement_"+str(interest)+".csv", index=False)
