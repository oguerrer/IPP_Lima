...
el presupuesto optimizado para el escenario fiscal conservador. Se guardan
...
dfbo = pd.read_csv(home+"data/optimal_budget.csv")
...
dfi.groupby('indicador').mean().to_csv(home+"data/sims/optimal_muni.csv", index=False)

dfp = pd.DataFrame(output_data_P, columns=['indicador']+list(range(len(serie))))
dfp.groupby('indicador').mean().to_csv(home+"data/sims/optimal_Ps_muni.csv", index=False)
