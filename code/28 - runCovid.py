...
el escenario Covid planteado en el reporte. Se guardan las dinámicas promedio
de los indicadores y las asignaciones presupuestales inferidas.
...
dfi.groupby('indicador').mean().to_csv(home+"data/sims/covid.csv", index=False)

dfp = pd.DataFrame(output_data_P, columns=['indicador']+list(range(len(serie))))
dfp.groupby('indicador').mean().to_csv(home+"data/sims/covid_Ps.csv", index=False)
